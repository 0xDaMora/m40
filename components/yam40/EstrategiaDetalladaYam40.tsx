...
import EstrategiaDetalladaTramites from "@/components/EstrategiaDetalladaTramites"
import PremiumUpsellSection from "@/components/PremiumUpsellSection/index"
import Yam40MejorasEstrategia from "./Yam40MejorasEstrategia"
import { calcularISRPension, calcularProyeccionPension, calcularPensionViudez } from "@/lib/all/calcularISR"
import html2canvas from "html2canvas"
import jsPDF from "jspdf"
import { useSession } from "next-auth/react"

interface EstrategiaDetalladaYam40Props {
  estrategia: {
    estrategia: string
    umaElegida: number
    mesesM40: number
...
const getTabs = (esMejora: boolean) => {
  const baseTabs = [
  ...
  // Solo agregar tab de mejoras si NO es una estrategia de mejora
  if (!esMejora) {
    baseTabs.splice(4, 0, { id: "mejoras", label: "🚀 Mejorar Estrategia", icon: TrendingUp })
  }
  
  return baseTabs
}

export default function EstrategiaDetalladaYam40({ estrategia, datosUsuario, onVolver, debugCode, familyMemberId }: EstrategiaDetalladaYam40Props) {
  ...
  // Detectar si es una estrategia de mejora (desde debugCode o estrategia.esMejora)
  const esMejora = estrategia.esMejora || (debugCode?.includes('_mejora_') ?? false)
  const tabs = getTabs(esMejora)
  ...
  // Fechas importantes - para yam40 usar fechas guardadas si están disponibles
  const fechaInicioM40 = estrategia.fechaInicioM40 
    ? new Date(estrategia.fechaInicioM40)
    : new Date(datosUsuario.inicioM40 || "2024-02-01")
  const fechaFinM40 = estrategia.fechaFinM40
    ? new Date(estrategia.fechaFinM40)
    : (() => {
        const fin = new Date(fechaInicioM40)
        fin.setMonth(fin.getMonth() + estrategia.mesesM40)
        return fin
      })()
  ...
  return (
    ...
    >
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <h1 className="text-xl md:text-3xl font-bold mb-2">
                {esMejora 
                  ? "🎯 Mejora de tu Estrategia" 
                  : estrategia.tipo === 'yam40' 
                    ? "🎯 Mi Modalidad 40 Actual"
      ...
      <div className="p-4 md:p-6">
        {/* Preservar estado: renderizar todos los tabs pero ocultar los inactivos */}
        <div className="relative">
          {tabs.map((tab) => {
            ...
            return (
              ...
              >
                ...
                {tab.id === "mejoras" && !esMejora && (
                  <Yam40MejorasEstrategia
                    key={`mejoras-${debugCode || 'default'}`}
                    estrategiaActual={estrategia}
                    datosUsuario={datosUsuario}
                    fechaInicioM40={fechaInicioM40}
                    fechaFinM40={fechaFinM40}
                    tipoPago={estrategia.tipoPago || 'aportacion'}
                    valorInicial={estrategia.valorInicial || (estrategia.registros && estrategia.registros.length > 0 ? estrategia.registros[0].cuotaMensual : 5000)}
                    sdiHistorico={estrategia.sdiHistorico || datosUsuario.sdiHistorico || 150}
                    semanasPrevias={estrategia.semanasPrevias || datosUsuario.semanasPrevias || 500}
                    edadJubilacion={typeof datosUsuario.edadJubilacion === 'number' ? datosUsuario.edadJubilacion : (typeof datosUsuario.edadJubilacion === 'string' ? parseInt(datosUsuario.edadJubilacion) : 65)}
                    dependiente={(estrategia.dependiente || datosUsuario.dependiente || 'ninguno') as 'conyuge' | 'ninguno'}
                    listaSDI={estrategia.listaSDI}
                  />
                )}
                {tab.id === "viudez" && (
                  <TabViudez 
