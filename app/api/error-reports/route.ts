...
const MAX_REQUESTS_PER_HOUR = 20 // Rate limiting: máximo 20 reportes por hora por email
...
async function checkRateLimit(email: string): Promise<boolean> {
  try {
    ...
    const recentReports = await prisma.errorReport.count({
      where: {
    ...
    return recentReports < MAX_REQUESTS_PER_HOUR
  } catch (error) {
    console.error("Error checking rate limit:", error)
    // En caso de error, permitir el reporte (fail open)
    return true
  }
}

// POST - Crear nuevo reporte de error
export async function POST(request: NextRequest) {
  try {
    ...
    if (!canProceed) {
      return NextResponse.json(
        { error: "Has enviado demasiados reportes. Por favor espera un momento antes de intentar nuevamente." },
        { status: 429 }
      )
    }
    ...
    if (error.code === "P2002") {
      return NextResponse.json(
        { error: "Ya existe un reporte con este correo electrónico" },
        { status: 409 }
      )
    }
